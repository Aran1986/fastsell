    ...
    <div className="min-h-screen bg-slate-50/50 flex flex-col items-center relative overflow-x-hidden" dir="rtl">
         ...
         {isChatOpen ? (
           <div className="bg-white rounded-[2.5rem] w-[320px] sm:w-[350px] h-[500px] shadow-2xl flex flex-col border border-slate-100 animate-in slide-in-from-bottom-5 duration-300 overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-50/50">
                 <div className="p-3 bg-indigo-50 text-indigo-700 text-[10px] font-black rounded-2xl text-center">سوالات خود را مستقیماً از ما بپرسید.</div>
                 {chatMessages.map(m => (
      ...
      <header className="w-full max-w-2xl pt-16 pb-12 px-6 text-center z-10">
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-3 max-w-md mx-auto">
            <div className="bg-white/60 backdrop-blur p-3 sm:p-4 rounded-[2rem] border border-white/50 shadow-sm">
                <div className="text-indigo-600 font-black text-lg sm:text-xl mb-1">⭐ {link.trustScore || '4.5'}</div>
                <div className="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase">امتیاز اعتبار</div>
            </div>
            <div className="bg-white/60 backdrop-blur p-3 sm:p-4 rounded-[2rem] border border-white/50 shadow-sm">
                <div className="text-green-600 font-black text-lg sm:text-xl mb-1">%{link.deliverySuccessCount ? '99' : '100'}</div>
                <div className="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase">ارسال موفق</div>
            </div>
            <div className="bg-white/60 backdrop-blur p-3 sm:p-4 rounded-[2rem] border border-white/50 shadow-sm">
                <div className="text-orange-600 font-black text-base sm:text-lg mb-1">~{link.avgResponseTimeMinutes || '30'}m</div>
      ...
      <main className="w-full max-w-2xl px-6 pb-24 space-y-8">
            ...
            <div key={product.id} className="group block bg-white rounded-[3rem] sm:rounded-[3.5rem] shadow-sm hover:shadow-2xl transition-all duration-500 border border-slate-100 overflow-hidden">
              <div className="flex flex-col sm:flex-row relative">
                  <div className="sm:w-56 h-56 overflow-hidden relative">
                      ...
                      {isOutOfStock && <div className="absolute inset-0 bg-slate-900/60 flex items-center justify-center"><span className="text-white font-black text-sm border-2 border-white px-4 py-2 rounded-xl transform -rotate-12">ناموجود</span></div>}
                  </div>
                  <div className="flex-1 p-6 sm:p-8 flex flex-col justify-between">
                      ...
                      <div className="mt-8 flex items-center justify-between">
                          <div className="flex flex-col">
                              <span className="text-[9px] font-black text-slate-300 uppercase">قیمت کالا</span>
                          ...
                          {isOutOfStock ? (
                            <button onClick={(e) => handleNotifyMe(e, product.id)} disabled={notifiedProds.has(product.id)} className={`px-6 sm:px-8 py-4 rounded-[1.8rem] text-xs sm:text-sm font-black shadow-lg transition-all ${notifiedProds.has(product.id) ? 'bg-green-100 text-green-600' : 'bg-slate-900 text-white'}`}>{notifiedProds.has(product.id) ? 'ثبت شد ✓' : 'موجود شد خبرم کن'}</button>
                          ) : (
                            <Link to={`/checkout/${link.slug}/${product.id}`} style={{ backgroundColor: buyBtnColor }} className="text-white px-6 sm:px-8 py-4 rounded-[1.8rem] text-xs sm:text-sm font-black shadow-lg transform group-hover:-translate-x-2 transition-all">خرید مستقیم</Link>
                          )}
                      </div>
                  </div>
              </div>
        ...
        <section className="bg-white rounded-[3rem] sm:rounded-[3.5rem] p-8 sm:p-10 shadow-sm border border-slate-100 mt-12">
            ...
            <div className="mb-12 p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100">
               <h4 className="font-black text-slate-800 text-sm mb-4">ثبت نظر خریدار (تایید شده)</h4>
               <form onSubmit={(e) => link.products[0] && handleSubmitReview(e, link.products[0].id)} className="space-y-4">
                  ...
                  <textarea rows={3} value={reviewText} onChange={e => setReviewText(e.target.value)} placeholder="تجربه خرید خود را بنویسید..." className="w-full px-5 py-4 rounded-2xl border border-slate-200 outline-none font-bold text-xs resize-none" />
                  ...
                  <button type="submit" disabled={isSubmittingReview} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black text-xs hover:bg-indigo-600 transition-all shadow-xl">
                    {isSubmittingReview ? 'در حال بررسی توسط آنتی‌اسپم هوشمند...' : 'تایید و ثبت نظر'}
                  </button>
               </form>
            </div>
